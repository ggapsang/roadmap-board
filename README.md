...
| 가로 폭 바꾸기 | 카드 좌·우 가장자리를 끈다. 되돌리려면 편집 패널의 **가로 폭 › 자동으로** |
...
| 화살표 굵기·글자 크기 | 툴바 **구성 › 표시** |
| 글자 크기 빠르게 | `Ctrl` `+` / `Ctrl` `-` / `Ctrl` `0`(원래대로) |
| 상태·조직으로 거르기 | 상단 요약 바의 칩을 누른다 |
| 찾기 | 상단 검색창 (`Ctrl+F`). 맞는 일정만 또렷하게 남는다 |
| 카드 글자 복사 | 툴바 **I** 버튼 (`Ctrl+Shift+S`) — 선택 모드. 켜는 동안 드래그 이동은 멈춘다 |
...
굵기와 머리 크기는 구성 패널에서 조절하고 문서에 함께 저장된다.
